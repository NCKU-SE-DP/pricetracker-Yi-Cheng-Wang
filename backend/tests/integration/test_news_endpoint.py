...
from src.auth.services import pwd_context
...
def mock_openai(mocker, return_content):
    mock_openai_client = mocker.patch('src.routers.news.OpenAIClient')
    try:
        mock_openai_client.return_value.generate_summary.return_value = json.loads(return_content)
    except:
        mock_openai_client.return_value.extract_search_keywords.return_value = return_content

def test_search_news(mocker):
    mock_openai(mocker, "keywords")

    mock_get_new_info = mocker.patch("src.routers.news.get_new_info", return_value=[
