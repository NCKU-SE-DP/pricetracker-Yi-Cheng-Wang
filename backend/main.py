...
class User(Base):
    ...
    upvoted_news = relationship(
        "NewsArticle",
        secondary=user_news_upvotes_table,
...
class NewsArticle(Base):
    ...
    upvoted_by_users = relationship(
        "User", secondary=user_news_upvotes_table, back_populates="upvoted_news"
    )
...
def fetch_and_process_news(is_initial=False):
    ...
    for news_item in news_data:
        ...
        if relevance_level == "high":
            ...
            article_publish_time = soup.find("time", class_="article-content__time").text
...
def get_article_upvote_details(article_id, user_id, database):
    upvote_count = (
        database.query(user_news_upvotes_table)
...
def read_news(database=Depends(session_opener)):
    ...
    :return:
    """
    news_articles = database.query(NewsArticle).order_by(NewsArticle.publish_time.desc()).all()
    result = []
...
def toggle_upvote(news_id, user_id, database):
    existing_upvote = database.execute(
        select(user_news_upvotes_table).where(
            user_news_upvotes_table.c.news_article_id == news_id,
            user_news_upvotes_table.c.user_id == user_id,
        )
    ).scalar()

    if existing_upvote:
        delete_stmt = delete(user_news_upvotes_table).where(
            user_news_upvotes_table.c.news_article_id == news_id,
            user_news_upvotes_table.c.user_id == user_id,
        )
        ...
        insert_stmt = insert(user_news_upvotes_table).values(
